...
function processMessageMentions(messageObj: any) {
  ...
  const replaceStr = (val: string): string => {
      ...
      if (eqTime > lastEqTime) {
        ...
        try {
          if (eqConfig.target_type === "private") {
            const actualCode = await resolveEmployeeCode(eqConfig.target_value);
            await sendPrivateMessage(actualCode, msgText);
          } else {
            await sendGroupMessage(eqConfig.target_value, msgText);
          }
